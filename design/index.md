...
| 日期 | 目录 | 状态 | 内容 |
|---|---|---|---|
| 2026-06-29 | [`20260629-agent-class-refactor`](20260629-agent-class-refactor/index.md) | 已实现，待验证 | `src/agent/harness.ts` 已拆为 `AgentService`、`HarnessFactory`、`AgentRuntime`、`BaseAgent` / `OneShotAgent` 和 `src/agent/agents/*`；`runForStream` / `runForFinalText` / `runForSideEffect` / `runForToolResult` 收敛一次性 agent 的结果提取；dm/topic/thread 走 `ChatAgent`，diary 走 `DiaryAgent`；distill/knowledge_index/consolidation/daily_memory/schedule 只搬运行形态到 agent 类，业务编排仍留在 `src/diary/*`、`src/memory/*`、`src/schedule/*`；画像编辑权限由 `toolGroups: ["profile_edit"]` 声明。 |
| 2026-06-29 | [`20260629-vault-refactor`](20260629-vault-refactor/index.md) | 需求定稿，待实现 | 知识库极简化重构：砍 Inbox/Garden 二元、`.index.md` 常驻 prompt、save_to_garden / promote / update_frontmatter / grep_vault 等老工具；新增收藏群 `LarkChatType: "clip"` 入口、`/save` 命令、`vault_digest` cron、`knowledge_policy.md`、`flash` 模型档位；vault 工具集收敛为 `vault_search` / `vault_read` / `fetch_article` / `vault_save` 四个；digest 子 agent 用 submit_tool 模式（即 `vault_save`），tag 词表 `_meta/tags.json` 软校验。替代 `20260619-mvp-to-north-star` 中的知识库章节。 |
| 2026-06-28 | [`20260628-session-resume-and-system-refresh`](20260628-session-resume-and-system-refresh/index.md) | 已实现，待验证 | 定义会话恢复与 system prompt 刷新：SQLite session/message 索引、JSONL transcript 恢复、reply_to 冷启动恢复规则、topic 长寿命会话、idle/new 边界、每轮刷新但保持 prompt 确定性。当前实现见 `docs/sessions.md`、`src/agent/service.ts`、`src/agent/sessions.ts`。 |
...
| 2026-06-21 | [`20260621-runtime-setting-final-form`](20260621-runtime-setting-final-form/index.md) | 已实现 | 定义 `setting.json` / `lark_config.json` / `schedules.json` 边界，把 LLM provider、时区、会话策略、script timeout、HTTP 与 knowledge index 周期纳入最终配置方案；当前事实见 `docs/configuration.md`。 |
...
1. 看知识库重构（收藏群入口、`/save`、digest 子 agent、工具集收敛、knowledge_policy）：读 [`20260629-vault-refactor`](20260629-vault-refactor/index.md)。
...
4. 看运行配置最终边界、LLM provider、时区、会话策略、script timeout、HTTP 与 knowledge index 周期：先读 [`20260621-runtime-setting-final-form`](20260621-runtime-setting-final-form/index.md)。
