...
- 续聊：内存 `Map` 里查得到这个 `scopeId`，复用现有 harness，只刷新最后活动时间。
- 新会话：查不到，新建 harness，并开一个全新的 JSONL 会话文件。
...
| 途径 | 触发条件 | 说明 |
|---|---|---|
| 空闲关闭 | 满足 `setting.sessions.policies` 对应类型的 `idleMinutes` | 主要冷却机制；关闭前会先蒸馏需要收尾的 scope |
...
后台按 `sweepIntervalMs` 扫一遍所有会话条目，按对应策略判断是否关闭：

- `autoClose: false`：不自动关闭，只能靠 `/new`、手动 compact 或进程重启收束。
...
因为是周期扫描，实际关闭时刻会比刚满 `idleMinutes` 晚最多一个 `sweepIntervalMs`。
