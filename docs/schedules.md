...
定时任务分两类：框架内置的 builtin，以及用户写的 script。配置文件是 `schedules.json`，路径见 [配置参考](configuration.md)。
...
| `daily-memory` | `daily_memory` | `0 6 * * *` | 处理前一上海自然日的 fresh episodes，维护 storylines，必要时轻触达 |
| `knowledge-index` | `knowledge_index` | 内容量触发 | 刷新 `vault/.index.md` 知识地图 |

cron 均按 `Asia/Shanghai` 解释。
...
- script 在 `worker_threads` 里运行，默认 60 秒超时。
- worker 设置 JS heap 限制；它不是安全沙箱，但能隔离常见错误、异常和 CPU 卡死。
...
`knowledge-index` 没有 cron 时也会被后台每小时检查一次。默认 trigger：
