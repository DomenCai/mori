...
- `scripts/install.mjs` — 全新安装：在仓库根装依赖、显式 build 并全局 link。
...
前置依赖：**Node ≥ 22.19**、**pnpm**（缺失推荐 `npm i -g pnpm`，也可用 npm 兜底）。文章抓取依赖 **defuddle CLI**；缺失时 `fetch_article` 会临时走 `npx defuddle`，首次抓取可能很慢或超时。`better-sqlite3` 默认下载预编译二进制，**通常不需要编译器**；仅当拿不到匹配二进制需本地编译时（全新 Node 大版本 / 冷门平台 / GitHub 下载受阻）才需 macOS 的 **Xcode CLT**。

⚠️ **飞书扫码注册必须用户前台手动完成**（`mori setup` 在终端渲染二维码），脚本无法代劳，只能引导。所有有副作用的动作（clone、装包、link、拉起 daemon、连通测试发真实请求、全局安装 defuddle）**先问用户再做**。
...
1. **补依赖**：doctor 报 Node 过低 / 缺 pnpm / 缺 Xcode CLT 时，告知用户处理（升级 Node、`npm i -g pnpm`、`xcode-select --install`），不擅自改全局环境。doctor 报 `defuddle 未安装` 时，询问用户是否帮他全局安装；同意后优先 `pnpm add -g defuddle`，无 pnpm 时用 `npm i -g defuddle`，装完重跑 doctor。
...
4. **引导首次配置**（用户手动）：`mori setup` → 选择 API 协议、填写 Base URL / key、选择模型、扫描飞书二维码，并按提示决定是否启动 daemon。
...
**非源码部署**（doctor 报 bin 指向不是 mori 仓库，即 npm 全局包）→ update.mjs 不适用，改重装：`npm install -g @domencai/mori@latest`，再 `mori stop && mori start`。
...
- `defuddle 未安装` → 说明文章抓取会退到 `npx defuddle`，首次抓取可能慢或超时；询问用户是否帮他全局安装。用户同意后执行 `pnpm add -g defuddle`（或无 pnpm 时 `npm i -g defuddle`），再重跑 doctor。
...
优先运行 `mori setup`：它会询问 API 类型、Base URL、key 和 normal / strong 模型，自动发现 endpoint 模型、写入价格并保存为 `MORI_API_KEY`。只有 setup 无法满足或用户明确要求手改时，才按下面直接编辑 `setting.json` 的 `llm.providers`。
...
手动配置时再问齐：**baseUrl、apiKeyEnv、model id、显示名、contextWindow / maxTokens（按厂商文档）**。
...
- **抓文章超时 / 首次很慢**：确认 `doctor.mjs` 的「文章抓取依赖」是否提示 `defuddle 未安装`；询问用户后全局安装 `pnpm add -g defuddle`，避免每次临时 `npx defuddle`。
- **`better-sqlite3` 安装报错**：预编译二进制没下到、回退本地编译又缺 Xcode CLT（`xcode-select --install`）；或 Node 版本没有对应 prebuild。
- **daemon 起不来**：多半 setup 未完成（`lark_config.json` 或 LLM 配置缺失）→ 先跑 `mori setup`，再看 `~/.mori/logs/$(date +%F).log`。
